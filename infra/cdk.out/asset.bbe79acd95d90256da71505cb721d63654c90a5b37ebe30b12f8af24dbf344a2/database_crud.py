...
def _get_db_config():
    ...
    try:
        ...
        if cfg and cfg.get("host") and cfg.get("username"):
            ...
            # Prefer password stored in the secret; only fall back to IAM token if missing.
            if not password:
                try:
                    rds = boto3.client("rds", region_name=rds_region)
                    password = rds.generate_db_auth_token(
                        DBHostname=host, Port=port,
                        DBUsername=username, Region=rds_region
                    )
                except Exception:
                    pass
            return {
    ...
    user = os.environ.get("DB_USER", "cdssadmin")
    
    # If we have a real host (not localhost), generate IAM token
    ...
    if host != "localhost" and not password:
        try:
            ...
            password = rds.generate_db_auth_token(
                DBHostname=host, Port=port, 
                DBUsername=user, Region=region
            )
        except Exception as e:
            print(f"IAM token generation failed: {e}")
...
def lambda_handler(event, context):
    ...
    # --- TEMP SEED ENDPOINT (for development only) ---
    if resource == '/seed':
        conn = get_db_connection()
        if not conn:
            return success_response({'error': 'no db connection'}, 500)
        try:
            conn.autocommit = True
            with conn.cursor() as cur:
                schema_sql = event.get('schema', '')
                seed_sql = event.get('seed', '')
                if schema_sql:
                    cur.execute(schema_sql)
                if seed_sql:
                    cur.execute(seed_sql)
            return success_response({'status': 'seeded successfully'})
        except Exception as e:
            return success_response({'error': str(e)}, 500)
        finally:
            conn.close()

    # --- HEALTH CHECK (must run before any other resource checks) ---
    if resource == '/health':
        conn = get_db_connection()
        try:
            if not conn:
                return success_response({"status": "ok", "database": "unavailable"})
            ...
            # Avoid leaking internal details to the caller; health is boolean-like.
            return success_response({"status": "ok", "database": "unavailable"})
        finally:
            try:
                if conn:
