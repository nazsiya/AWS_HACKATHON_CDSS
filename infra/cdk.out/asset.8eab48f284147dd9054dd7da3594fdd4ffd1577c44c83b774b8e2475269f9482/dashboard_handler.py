...
def get_db_connection():
    secret_name = os.environ.get('RDS_CONFIG_SECRET_NAME', '')
    if secret_name:
        try:
            secret = boto3.client('secretsmanager',
                region_name=os.environ.get('AWS_REGION','ap-south-1')
            ).get_secret_value(SecretId=secret_name)
            cfg = _json.loads(secret['SecretString'])
            return psycopg2.connect(
                host=cfg['host'], port=cfg['port'],
                database=cfg['dbname'], user=cfg['username'],
                password=cfg['password'], sslmode='require',
                connect_timeout=10
            )
        except Exception as e:
            logger.error(f"Secret connection failed: {e}")
    return psycopg2.connect(
    ...
    if resource == '/seed':
        ...
        try:
            ...
            with conn.cursor() as cur:
                if event.get('schema'): cur.execute(event['schema'])
                if event.get('seed'): cur.execute(event['seed'])
